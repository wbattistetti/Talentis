...
const Vision: React.FC = () => {
  ...
  const visionSteps: VisionStep[] = [
    {
      icon: IMAGES.horse,
      title: t('vision.steps.0.title'),
      description: t('vision.steps.0.description'),
      keywords: t('vision.steps.0.keywords', { returnObjects: true }) as string[]
    },
    {
      icon: IMAGES.migration,
      title: t('vision.steps.1.title'),
      description: t('vision.steps.1.description'),
      keywords: t('vision.steps.1.keywords', { returnObjects: true }) as string[]
    },
    {
      icon: IMAGES.ai,
      title: t('vision.steps.2.title'),
      description: t('vision.steps.2.description'),
      keywords: t('vision.steps.2.keywords', { returnObjects: true }) as string[]
    },
    {
      icon: IMAGES.ecosystem,
      title: t('vision.steps.3.title'),
      description: t('vision.steps.3.description'),
      keywords: t('vision.steps.3.keywords', { returnObjects: true }) as string[]
    }
  ];
  ...
  useEffect(() => {
    ...
    if (lastStep !== undefined && lastStep < visionSteps.length - 1) {
      const timer = setTimeout(() => {
        setActiveSteps(prev => [...prev, lastStep + 1]);
      }, 5000);
      return () => clearTimeout(timer);
    } else if (lastStep === visionSteps.length - 1) {
      const timer = setTimeout(() => {
        startKeywordHighlighting();
      }, 2000);
      return () => clearTimeout(timer);
    }
  }, [activeSteps, visionSteps.length]);

  const startKeywordHighlighting = () => {
    let currentPanel = 0;
    
    const highlightNextPanel = () => {
      if (currentPanel >= visionSteps.length) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 to-blue-900 text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        <div className={`text-center mb-8 transition-all duration-1000 ${isHeaderVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-12'}`}>
          <h1 className="text-4xl md:text-5xl font-bold mb-3">{t('vision.title')}</h1>
          <p className="text-xl text-blue-200 max-w-3xl mx-auto">{t('vision.subtitle')}</p>
        </div>
